...
public static class MaterialFactory
{
    ...
    public static StandardMaterial3D Get(string key, Color tint, float roughness = 0.85f,
        float metallic = 0f, bool occluder = false)
    {
        if (Cache.TryGetValue(key, out var cached))
            return cached;

        var noise = new FastNoiseLite { Frequency = 0.08f, FractalOctaves = 3 };
        var tex = new NoiseTexture2D { Noise = noise, Width = 256, Height = 256 };
    ...
    // Palette helpers (Vision 6.10).
    public static StandardMaterial3D FloorStone() => Get("floor_stone", Color.FromHtml("5a5a5e"), 0.9f);
    public static StandardMaterial3D WallStone() => Get("wall_stone", Color.FromHtml("5a5a5e"), 0.85f, occluder: true);
    public static StandardMaterial3D PlayerSteel() => Get("player_steel", Color.FromHtml("8a919c"), 0.45f, metallic: 0.5f);
}
